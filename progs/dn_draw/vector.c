...
A series of subroutines designed to carry out common 3-vector operations
	...
	double a[],b[],result[];
					Return the 3-vector result which is the vector b in cylintrical coords
	...
	struct basis *basis_new;		
					creates an orthonormal basis with the first axis in the u direction, the
...
What is rotated and unrotated is likely to be confusing.  This may help.  Suppose you are working in frame1, and you
want to get a quantity in frame2.  Then you need to start by calling create_basis.  
	create_basis(x,y,&base);
where x is now a vector in frame1 that you want to point along the x-axis of frame2 and y defines the x' y' plane

Now if you want to project from frame1 to frame2, you will use 
         project_to(&base,x1,x2);

and if you want to project from frame2 to frame 1, you will use
         project_from(&base,x2,x1);
...
/* Project a vector b from position a into cylintrical coordinates */
     ...
     double a[], b[], result[];
{
  ...
  if (renorm (n_rho, 1.0) == -1)
    {
      printf ("Position on z axis; converstion cylintrical coords indeterminate\n");
      return (-1);
    }
  n_z[0] = n_z[1] = 0;
  n_z[2] = 1;
