...
float reg_weight[NREG], frac_weight[NREG];
     ...
     char *argv[];
{
  ...
  char one_ok[LINELENGTH];
  char two_ok[LINELENGTH];
  ...
  if (argc == 1)
      ...
      for (i = 1; i < argc; i++)
	{
	  if (strcmp (argv[i], "-h") == 0)
	    {
	      printf
		("Usage: py_compare [-m1 firstmodel nspec -m2 secondmodel nspec -s shift] pffile\n");
	      exit (0);
	    }
		...
		{
		  Error
		    ("py_compare: Expected second model after -m2 switch\n");
		  exit (0);
		}
	      i++;
		...
		{
		  Error
		    ("py_compare: Expected spectrum no after modelname\n");
		  exit (0);
		}
	      i++;
	    }
		...
		{
		  Error
		    ("py_compare: Expected integer to indicate shift after  -s switch\n");
		  exit (0);
		}
	      i++;
	    }
...
/* End of input section */
  ...
  /* Report the desired files are missing if that is the case */

  if ((strcmp (one_ok, "nok") == 0) || (strcmp (two_ok, "nok") == 0))
    {
      fprintf (optr, "# Missing  %s  %d  %3s    %s  %d  %3s\n", first_model, first_spec,
	       one_ok, second_model, second_spec,two_ok);
      Error ("# Missing  %s    %3s    %s    %3s\n", first_model, one_ok,
	     second_model, two_ok);
  ...
  for (n = 0; n < nreg; n++)
    {
      ...
      x = metric (&first_spectrum, &second_spectrum,shift);
  ...
  fprintf (optr, "%20s %2d  %20s %2d %8.3f", first_model, first_spec,
	   second_model, second_spec, overall);
  for (n = 0; n < nreg; n++)
    {
...
/* 
 * 090608  Average first before calculating the differences
 * 090609  Add the possibility of adding shifts to the spectrum
 * 		in order to try to estimate the noise in the spectrum
 */

float
metric (one, two,shift)
     SpecPtr one, two;
     int shift;
{
  int n;
  ...
  for (n = 0; n < one->nw; n++)
    {
      if (one->good[n])
	{
	  x += fabs (one->f[n] - ratio*two->f[n+shift]);
	}
    }
