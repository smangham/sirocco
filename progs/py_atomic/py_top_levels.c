...
energy levels.  
  ...
  History:
	...
	01dec	ksl	Allowed for lines without radiative lifetimes.
			...
			radiative lifetime.
        12sep	nsh	included stdlib.h in the include statements to 	
			avoid compile error for exit(). Also changed
			nlev and index to be unsigned long - the code 
			did not run due to the mismatched variables.
...
//Allow for the possibility of no rl  01dec12 ksl
	...
	printf ("We have read in %i levels\n",nlev);
for (i=0;i<nlev;i++)
	{
	printf ("i=%i,excitiation above ground=%e\n",i,config[i].ex);
	}
  sortem (nlev);
     ...
     unsigned long nlev;
{
  float x[NLEVELS + 1];
  int i ;
  unsigned long iorder[NLEVELS + 1];
...
// Now rearrange the indices
  for (i = 0; i < nlev; i++)
    {
	printf ("1 i= %i iorder[i+1]-1=%i\n",i,iorder[i+1]-1);
	printf ("test=%i\n",yhold[iorder[i+1]-1]->istate);
